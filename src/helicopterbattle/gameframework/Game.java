   ...
   /**
    ...
    private void createEnemyHelicopter(long gameTime) {
        if(gameTime - EnemyHelicopter.timeOfLastCreatedEnemy >= EnemyHelicopter.timeBetweenNewEnemies) {
            ...
            int yCoordinate = random.nextInt(Framework.frameHeight - (EnemyHelicopter.helicopterBodyImg.getHeight())*3);
    ...
    private void createEnemyTank(long gameTime) {
        if(gameTime - EnemyTank.timeOfLastCreatedEnemy >= EnemyTank.timeBetweenNewEnemies) {
           ...
           // int yCoordinate = Framework.frameHeight - Framework.frameHeight - EnemyTank.tankBodyImg.getHeight();
     ...
     * Move the Tank and checks if he left the screen.
	...
	private void updateEnemyTanks() {
		for (int i = 0; i < enemyTankList.size(); i++) {
            ...
            // Is crashed with player?
            Rectangle playerRectangel = new Rectangle(player.xCoordinate, player.yCoordinate, player.helicopterBodyImg.getWidth(), player.helicopterBodyImg.getHeight());
            Rectangle enemyRectangel = new Rectangle(et.xCoordinate, et.yCoordinate, EnemyTank.tankBodyImg.getWidth(), EnemyTank.tankBodyImg.getHeight());
            if(playerRectangel.intersects(enemyRectangel)) {
                player.health = 0;
            ...
            if(et.health <= 0) {
                ...
                Animation expAnim = new Animation(explosionAnimImg, 134, 134, 12, 45, false, et.xCoordinate, et.yCoordinate - explosionAnimImg.getHeight()/3, 0); // Substring 1/3 explosion image height (explosionAnimImg.getHeight()/3) so that explosion is drawn more at the center of the tank.
                explosionsList.add(expAnim);
    ...
    private void isTankShooting(long gameTime, EnemyTank tank)
    {
       if( tank.isShootingTime(gameTime) )
       {
          ...
          tank.numberOfAmmo--;
            ...
            Bullet b = new Bullet(tank.xCoordinate, tank.yCoordinate, new Point(player.machineGunXcoordinate, player.machineGunYcoordinate), true);
            bulletsList.add(b);
       }
    }
    /**

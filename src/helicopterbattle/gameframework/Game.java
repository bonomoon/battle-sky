...
import helicopterbattle.game.Animation;
...
import helicopterbattle.game.RocketSmoke;
...
public class Game {
	Bgm gameOver = new Bgm("GameOver.mp3", false);
	...
	public static enum FlightState {
		HELICOPTER1, HELICOPTER2, JETPLANE, AIRBALLOON
	}
	...
	private void LoadContent() {
		...
		mountainsMoving.Initialize(mountainsImg, -1,
				Framework.frameHeight - groundImg.getHeight() - mountainsImg.getHeight() + 40);
	...
	private void createEnemyHelicopter(long gameTime) {
		if (gameTime - EnemyHelicopter.timeOfLastCreatedEnemy >= EnemyHelicopter.timeBetweenNewEnemies) {
			...
			int yCoordinate = random
					.nextInt(Framework.frameHeight - (EnemyHelicopter.helicopterBodyImg.getHeight()) * 3);
	...
	private void createEnemyTank(long gameTime) {
		if (gameTime - EnemyTank.timeOfLastCreatedEnemy >= EnemyTank.timeBetweenNewEnemies) {
			...
			int xCoordinate = Framework.frameWidth;
			int yCoordinate = Framework.frameHeight - EnemyTank.tankBodyImg.getHeight();
			// int yCoordinate = Framework.frameHeight - Framework.frameHeight -
			...
			// Sets new time for last created enemy.
			EnemyTank.timeOfLastCreatedEnemy = gameTime;
		}
	}

	/**
	 * Updates all enemies. Move the helicopter and checks if he left the screen.
	 * Move the Tank and checks if he left the screen. Updates helicopter
	 * animations. Checks if enemy was destroyed. Checks if any enemy collision with
	 * player.
	 */
	...
	private boolean isCrashedPlayer(EnemyHelicopter eh, int enemyNum) {
		...
		Rectangle enemyRectangel = new Rectangle(eh.xCoordinate, eh.yCoordinate,
				EnemyHelicopter.helicopterBodyImg.getWidth(), EnemyHelicopter.helicopterBodyImg.getHeight());
		...
		for (int i = 0; i < bulletsList.size(); i++) {
			...
			if (bullet.isEnemy) {
				...
				if (bulletRectangle.intersects(playerRectangel)) {
					// 체력 감소
					player.health -= Bullet.EnemyDamagePower;

					// 총알 삭제
					bulletsList.remove(i);
				}
				continue; // 이 총알은 적군의 총알이므로 다음 경우는 보면 안됌
			}
			...
			for (int j = 0; j < enemyTankList.size(); j++) {
				EnemyTank et = enemyTankList.get(j);

				// Current enemy rectangle.
				Rectangle enemyRectangel = new Rectangle(et.xCoordinate, et.yCoordinate,
						EnemyTank.tankBodyImg.getWidth(), EnemyTank.tankBodyImg.getHeight());
	...
	private void isTankShooting(long gameTime, EnemyTank tank) {
		if (tank.isShootingTime(gameTime)) {
			Bullet.timeOfLastCreatedEnemyBullet = gameTime;
			tank.numberOfAmmo--;

			Bullet b = new Bullet(tank.xCoordinate, tank.yCoordinate,
					new Point(player.machineGunXcoordinate, player.machineGunYcoordinate), true);
			bulletsList.add(b);
		}
	}
	...
	private void updateRockets(long gameTime) {
		for (int i = 0; i < rocketsList.size(); i++) {
			...
			int smokePositionX = 5 + random.nextInt(8); // We will draw this smoke a little bit ahead of the one we draw
														// before.
			rs = new RocketSmoke();
			xCoordinate = rocket.xCoordinate - RocketSmoke.smokeImg.getWidth() + smokePositionX; // Here we need to add
																									// so that the smoke
																									// will not be on
																									// the same x
																									// coordinate as
																									...
																									// same coordinate
																									// as smoke before.
			yCoordinte = rocket.yCoordinate - 5 + random.nextInt(6); // Subtract 5 so that smok will be at the middle of
																		// the rocket on y coordinate. We rendomly add a
																		// number between 0 and 6 so that the smoke line
																		// isn't straight line.
			rs.Initialize(xCoordinate, yCoordinte, gameTime, rocket.currentSmokeLifeTime);
			rocketSmokeList.add(rs);
